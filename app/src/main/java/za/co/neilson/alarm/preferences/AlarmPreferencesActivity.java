		...
		getListView().setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> l, View v, int position, long id) {
				...
				case MULTIPLE_LIST_CHARITY:
					...
					for (Alarm.Charity charity : getMathAlarm().getCharities()) {
						checkedItems[charity.ordinal()] = true;
					}
					alert.setMultiChoiceItems(multiListItems, checkedItems, new OnMultiChoiceClickListener() {
						...
						public void onClick(final DialogInterface dialog, int which, boolean isChecked) {
							...
							Alarm.Charity thisCharity = Alarm.Charity.values()[which];

							if (isChecked) {
								alarm.addCharity(thisCharity);
							} else {
								// Only remove the day if there are more than 1
								// selected
								if (alarm.getCharities().length > 1) {
									alarm.removeCharity(thisCharity);
								} else {
					...
					case DOUBLE:
						...
						// Set an EditText view to get user input
						final EditText numinput = new EditText(AlarmPreferencesActivity.this);

						numinput.setText(alarmPreference.getValue().toString());

						alert.setView(numinput);
						alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int whichButton) {
								alarmPreference.setValue(numinput.getText().toString());

								if (alarmPreference.getKey() == Key.ALERT_AMOUNT) {
									try{
										alarm.setDonateAmount((Double) Double.parseDouble((alarmPreference.getValue().toString())));
									}
									catch(Exception e){
