...
public class Database extends SQLiteOpenHelper {
	...
	public static final String COLUMN_ALARM_CHARITIES = "alarm_charities";
	public static final String COLUMN_ALARM_AMOUNT = "alarm_amount";
	...
	public static long create(Alarm alarm) {
		...
		try {
			...
			oos2.writeObject(alarm.getCharities());
			byte[] buff = bos2.toByteArray();

			cv.put(COLUMN_ALARM_CHARITIES, buff);

		} catch (Exception e){
		}

		cv.put(COLUMN_ALARM_AMOUNT, alarm.getDonateAmount());
	...
	public static Alarm getAlarm(int id) {
		...
		if(c.moveToFirst()){
			...
			byte[] charitiesBytes = c.getBlob(5);
			ByteArrayInputStream byteArrayInputStreamCharity = new ByteArrayInputStream(charitiesBytes);
			try {
				ObjectInputStream objectInputStream = new ObjectInputStream(byteArrayInputStreamCharity);
				Alarm.Charity[] repeatCharities;
				Object object = objectInputStream.readObject();
				if(object instanceof Alarm.Charity[]){
					repeatCharities = (Alarm.Charity[]) object;
					alarm.setCharities(repeatCharities);
				}
			} catch (StreamCorruptedException e) {
				e.printStackTrace();
			} catch (IOException e) {
			...
			alarm.setDonateAmount(c.getDouble(6));
						
			alarm.setDifficulty(Difficulty.values()[c.getInt(7)]);
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE IF NOT EXISTS " + ALARM_TABLE + " ( " 
				...
				+ COLUMN_ALARM_CHARITIES + " BLOB NOT NULL, "
				+ COLUMN_ALARM_AMOUNT + " INTEGER NOT NULL, "
	...
	public static List<Alarm> getAll() {
		...
		if (cursor.moveToFirst()) {
			...
			do {
				...
				byte[] charitiesBytes = cursor.getBlob(4);
				ByteArrayInputStream byteArrayInputStreamCharity = new ByteArrayInputStream(charitiesBytes);
				try {
					ObjectInputStream objectInputStream = new ObjectInputStream(byteArrayInputStreamCharity);
					Alarm.Charity[] repeatCharities;
				...
				alarm.setDonateAmount(cursor.getDouble(5));

				alarm.setDifficulty(Difficulty.values()[cursor.getInt(6)]);
				alarm.setAlarmTonePath(cursor.getString(7));
